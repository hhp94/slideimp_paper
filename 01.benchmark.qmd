...
# Speed/Memory
Perform imputation on just 1000 CpGs of chr22 of MSA chip
```{r}
library(missRanger)

MSA_chr22 <- t(qs_read(get_DNAm("MSA", glue::glue("chr{22}"))$path)[1:1000, ])
MSA_chr22_miss <- is.na(MSA_chr22)
MSA_chr22 <- MSA_chr22[, colSums(MSA_chr22_miss) / nrow(MSA_chr22_miss) < 0.9]
sum(MSA_chr22_miss) * 100 / length(MSA_chr22_miss)

set.seed(1234)
if (!file.exists("benchmark/rf_bench.rds")) {
  rf_bench <- bench::mark(
    missRanger(
      as.data.frame(MSA_chr22),
      num.threads = 6,
      data_only = TRUE
    ),
    knn_imp(MSA_chr22, k = 20, cores = 6, weighted = TRUE, dist_pow = 1),
    check = F
  )
  rf_bench$memory <- NULL
  saveRDS(rf_bench, "benchmark/rf_bench.rds")
} else {
  rf_bench <- readRDS("benchmark/rf_bench.rds")
}

if (!file.exists("benchmark/rf_imp.rds")) {
  rf_fns <- function(obj, ...) {
    r <- missRanger(as.data.frame(obj), num.threads = 6, data_only = TRUE)
    as.matrix(r)
  }
  ...
  knn_imp_res <- tune_imp(
    MSA_chr22,
    .f = "knn_imp",
    parameters = data.frame(k = 20, weighted = TRUE, dist_pow = 3),
    rep = 5,
    cores = 6,
    num_na = 6000
  )

  saveRDS(rf_imp, "benchmark/rf_imp.rds")
  saveRDS(knn_imp_res, "benchmark/knn_imp_res.rds")
} else {
  rf_imp <- readRDS("benchmark/rf_imp.rds")
  knn_imp_res <- readRDS("benchmark/knn_imp_res.rds")
}

met_set <- yardstick::metric_set(yardstick::mae, yardstick::rmse, yardstick::rsq)

rf_imp$results <- lapply(
  rf_imp$result,
  \(x) {
...
knn_imp_res <- unnest(knn_imp_res, cols = "results")
subset(knn_imp_res) |>
  summarize(mean = mean(.estimate), .by = ".metric")

rf_imp <- unnest(rf_imp, cols = "results")
subset(rf_imp) |>
  summarize(mean = mean(.estimate), .by = ".metric")
