...
for i in {1..5}; do
  for j in 1 4; do
    /usr/bin/time -f "%U\t%S\t%e\t%P\t%M" -o output/baseline-10000-${i}-${j}.txt apptainer exec $APPTAINER/missRanger/missRanger.sif Rscript memory.R --rds input/functions.rds --chr1 input/EPIC_chr1_t.rds --fun_name f_knn_brute --baseline --cpgs 10000 --cores ${j}
  done
done

srun --partition=day --cpus-per-task=4 --mem=30GB --time=5:00
```
