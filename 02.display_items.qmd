...
EPIC <- tibble(
  platform = "EPIC",
  n = c(500, 1000, 5000, 10000, 20000, 50000, 72000),
...
EPIC_speed_plot <- EPIC_speed |>
  ...
  scale_x_continuous(
    breaks = c(500, 10000, 20000, 50000, 72000),
    labels = scales::comma
  ) +
...
multi_cores_output |>
  filter(!method %in% c("impute.knn", "methyLImp2_fork")) |>
  ggplot(aes(x = cores, y = as.numeric(median_elapsed), color = method_label)) +
  geom_point() +
  scale_y_log10() +
  geom_text_repel(aes(label = as.numeric(median_elapsed))) +
  geom_line(aes(group = method))

multi_cores_output |>
  filter(!method %in% c("impute.knn", "methyLImp2_fork", "knn_tree")) |>
  ggplot(aes(x = cores, y = median_max_rss_gb, color = method_label)) +
  geom_point() +
  # geom_text_repel(aes(label = median_max_rss_gb)) +
  geom_line(aes(group = method))
```
...
results$path <- with(results, glue::glue("benchmark/accuracy/{prefix}.{rate}.fst"))
...
EPIC_accuracy_plot <- results |>
  ...
  mutate(
    ...
    package = recode_values(package, "knn" ~ "knn_imp", default = package)
  ) |>
